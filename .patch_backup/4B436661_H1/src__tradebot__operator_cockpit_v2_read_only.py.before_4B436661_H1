...
OPERATOR_COCKPIT_V2_HYP006_VISUALIZATION_EXPORT_GUARD_HOTFIX_VERSION = "4B.4.3.6.6.28F-H4"
OPERATOR_COCKPIT_V2_HYP006_CUMULATIVE_PROGRESS_VISUAL = True
OPERATOR_COCKPIT_V2_RISK_SIZING_EXPORT_AVAILABILITY_GUARD = True
...
<div id="execution-panel" class="tab-panel"><div class="chart-grid"><div class="chart-card wide"><div class="chart-title">MAE / MFE execution görünümü</div><div class="chart-note">HYP-006 no-order için true execution MAE/MFE yoksa final edge proxy scatter çizilir.</div><div class="chart" id="mae-mfe-chart"></div></div></div></div></section>
...
function lineChart(el,data,xKey,yKey){if(!data.length){$(el).innerHTML=empty('Grafik için veri bulunamadı.');return}const yValue=x=>Number(x[yKey]??x.sample_count??0);const vals=data.map(yValue),max=Math.max(...vals,1),min=Math.min(...vals,0),span=Math.max(max-min,1);const px=i=>40+(i*Math.max(0,680/(Math.max(data.length-1,1)))),py=v=>175-((v-min)/span)*140;let body='<line class="axis" x1="40" y1="175" x2="730" y2="175"/><line class="axis" x1="40" y1="25" x2="40" y2="175"/>';for(let i=0;i<4;i++){const y=35+i*42;body+=`<line class="gridline" x1="40" y1="${y}" x2="730" y2="${y}"/>`}const points=data.map((x,i)=>`${px(i)},${py(yValue(x))}`).join(' ');body+=`<polyline class="line" points="${points}"/>`;data.forEach((x,i)=>{body+=`<circle class="point" cx="${px(i)}" cy="${py(yValue(x))}" r="4"><title>${esc(x[xKey])}: ${fmt(yValue(x),0)} sample</title></circle>`});body+=`<text x="40" y="198">${esc(String(data[0][xKey]).slice(0,10))}</text><text x="650" y="198">${esc(String(data[data.length-1][xKey]).slice(0,10))}</text>`;$(el).innerHTML=svg(body)}
