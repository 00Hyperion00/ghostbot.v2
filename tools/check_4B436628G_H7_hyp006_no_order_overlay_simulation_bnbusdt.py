...
def build_report(root: Path) -> dict[str, object]:
    ...
    module_path = root / "src/tradebot/hyp006_no_order_overlay_simulation_bnbusdt.py"
    ...
    checks = {
        ...
        "source_h6_contract_present": "SOURCE_H6_CONTRACT_VERSION" in module_text,
        "bnbusdt_primary_measurement_present": "BNBUSDT_PRIMARY_SYMBOL_FILTER_SHADOW_MEASUREMENT" in module_text,
        "overlay_simulation_report_present": "build_no_order_overlay_simulation_bnbusdt_report" in module_text,
        "quarantine_exclusion_present": "excluded_quarantine_candidates" in module_text,
        "watchlist_exclusion_present": "excluded_watchlist_candidates" in module_text,
        "do_not_relax_enforced_present": "enforced_do_not_relax_blocklist" in module_text,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 4B.4.3.6.6.28G-H7 HYP-006 no-order overlay simulation patch")
    ...
    if args.once_json:
        ...
        print(f"{CONTRACT_VERSION} HYP-006 no-order overlay simulation patch check")
