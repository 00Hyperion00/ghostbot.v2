...
def candidate_paths() -> list[Path]:
    paths: list[Path] = []
    parent = ROOT.parent
    for backup in sorted(parent.glob('trade_botV2_BACKUP*'), key=lambda p: p.stat().st_mtime, reverse=True):
        path = backup / 'src' / 'tradebot' / 'engine.py'
        if path.exists():
            paths.append(path)
    for backup in sorted(parent.glob('*BACKUP*'), key=lambda p: p.stat().st_mtime, reverse=True):
        path = backup / 'src' / 'tradebot' / 'engine.py'
...
def pick_backup_engine() -> tuple[Path, str, int]:
    ...
    if best:
        raise RuntimeError(f'No complete backup engine.py found. Best candidate={best[0]} score={best[2]}/{len(CORE_REQUIRED_METHODS)}')
    raise RuntimeError('No backup engine.py found. Restore a BACKUP_4B436611i/4B436612a folder or upload a full project zip.')


def backup_current_engine() -> Path:
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    target = ENGINE.with_suffix(f'.broken_before_4B436612c_{timestamp}.py')
...
def main() -> int:
    ...
    if current_score >= len(CORE_REQUIRED_METHODS):
        ...
        print(f'4B.4.3.6.6.12c detected truncated/corrupt engine.py score={current_score}/{len(CORE_REQUIRED_METHODS)}')
    ...
    if not all(value for key, value in final_checks.items() if key not in {'restored_from_backup', 'source_path', 'method_score'}):
        raise SystemExit(f'4B.4.3.6.6.12c engine verification failed: {final_checks}')

    api = patch_optional_file(API)
    dashboard = patch_optional_file(DASHBOARD)
    obs_test = patch_optional_file(RUNTIME_OBS_TEST)

    print('4B.4.3.6.6.12c engine integrity restore hotfix applied')
