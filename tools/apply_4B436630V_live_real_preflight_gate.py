...
def _run_checker(root: Path) -> dict[str, Any]:
    ...
    proc = subprocess.run(
        [sys.executable, str(root / "tools/check_4B436630V_live_real_preflight_gate.py"), "--once-json"],
        cwd=root,
        text=True,
...
def main() -> int:
    ...
    print("4B.4.3.6.6.30V live-real preflight gate applied")
    checks = checker_report.get("checks", {}) if isinstance(checker_report.get("checks"), dict) else {}
    for key in (
        "base_30u_checker_ok",
        "module_probe_ok",
        "module_probe_source_30u_ok",
        "module_probe_api_env_capability_audit_ok",
        "module_probe_account_capability_audit_ok",
        "module_probe_hard_submit_block_ok",
        "module_probe_no_exchange_submit_ok",
        "module_probe_no_live_real_order_ok",
    ):
