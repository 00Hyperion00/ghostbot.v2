...
from tradebot.research_hyp004_relative_strength_refinement import (  # noqa: E402
...
def http_get_json(base_url: str, path: str, params: dict[str, Any], timeout_sec: float) -> Any:
    ...
    request = Request(url, headers={"User-Agent": "tradebot-25P-hyp004-research/1.0"}, method="GET")
...
def latest_25o_report(reports_dir: Path) -> Path | None:
    matches = sorted(reports_dir.glob("4B436625O_hyp004_cross_symbol_relative_strength_exploration_*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    return matches[0] if matches else None


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="4B.4.3.6.6.25P HYP-004 relative strength candidate refinement / approvable strategy gate")
    parser.add_argument("--input-json", default=None, help="25O HYP-004 exploration report.")
    parser.add_argument("--input-csv", default=None, help="Offline CSV with symbol/open_time/close columns.")
    parser.add_argument("--reports-dir", default="reports", help="Directory used to discover latest 25O report if --input-json is omitted.")
    parser.add_argument("--symbols", default="BTCUSDT,ETHUSDT,SOLUSDT,BNBUSDT", help="Comma-separated symbols.")
    parser.add_argument("--interval", default="4h", help="Single interval for cross-symbol alignment.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.review_ok:
        print("ERROR: --review-ok is required. HYP-004 refinement is research-only and cannot approve paper/live trading.", file=sys.stderr)
        return 2
    input_json = Path(args.input_json) if args.input_json else latest_25o_report(Path(args.reports_dir))
    exploration_report = load_json(input_json) if input_json and input_json.exists() else None
    symbols = [s.strip().upper() for s in args.symbols.split(",") if s.strip()]
    if len(symbols) < 2:
        print("ERROR: at least two symbols are required for HYP-004 refinement", file=sys.stderr)
        return 2
    if args.input_csv:
    ...
    next_spec_path: Path | None = None
    if report.get("next_candidate_spec"):
        next_spec_path = out_dir / f"4B436625P_hyp004_next_candidate_for_25Q_{stamp}.json"
        write_json(next_spec_path, report["next_candidate_spec"])
    selected = report.get("selected_candidate") or {}
    metrics = selected.get("metrics", {}) if isinstance(selected, dict) else {}
    print(f"{HYP004_REFINEMENT_CONTRACT_VERSION} HYP-004 relative strength refinement {report['decision']}")
    print(f" - hypothesis_id: {report.get('hypothesis_id')}")
    print(f" - symbols: {','.join(report.get('symbols', []))}")
    print(f" - selected_25o_family: {report.get('selected_25o_family')}")
    print(f" - candidate_count: {report.get('candidate_count')}")
    print(f" - passed_candidate_count: {report.get('passed_candidate_count')}")
    print(f" - selected_refinement_name: {selected.get('refinement_name') if isinstance(selected, dict) else None}")
    ...
    if next_spec_path:
        print(f"next_candidate_25q_json: {next_spec_path}")
    return 0
