...
CONFIG_BLOCK = """
    ...
    paper_sandbox_execution_reconciliation_tolerance: float = 1e-9
    paper_sandbox_execution_reconciliation_sqlite_path: str = "reports/production_hardening/4B436630O_paper_execution_reconciliation_audit_mirror.db"
"""
PY_FILES = [
...
def _patch_config(root: Path) -> dict[str, Any]:
    ...
    missing = [field for field in CONFIG_FIELDS if field not in text]
    patched = False
    if missing:
    ...
    return {"patched": patched, "before_missing": missing, "after_missing": [field for field in CONFIG_FIELDS if field not in after]}


def _compile(root: Path) -> dict[str, dict[str, Any]]:
    out: dict[str, dict[str, Any]] = {}
    for rel in PY_FILES + ["src/tradebot/config.py"]:
        try:
...
def _run_h6(root: Path) -> dict[str, Any]:
    env = os.environ.copy()
    src_path = str(root / "src")
    env["PYTHONPATH"] = src_path if not env.get("PYTHONPATH") else f"{src_path}{os.pathsep}{env['PYTHONPATH']}"
    proc = subprocess.run([sys.executable, str(root / "tools/check_4B436630O_H6_reconciliation_module_checker_final.py"), "--once-json"], cwd=root, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env, check=False, timeout=300)
    try:
...
def main() -> int:
    ...
    print("4B.4.3.6.6.30O-H6 reconciliation module/checker final applied")
    checks = checker.get("checks", {}) if isinstance(checker.get("checks"), dict) else {}
    for key in ("target_30o_checker_ok", "h1_checker_ok", "h2_checker_ok", "h3_checker_ok", "h4_checker_ok", "h5_checker_ok", "target_ledger_consumed", "target_sqlite_mirror_ok", "target_exchange_submit_blocked", "target_live_real_blocked"):
