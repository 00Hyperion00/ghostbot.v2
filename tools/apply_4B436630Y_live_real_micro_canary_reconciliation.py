...
def _run_checker(root: Path) -> dict[str, Any]:
    ...
    proc = subprocess.run([sys.executable, str(root / "tools/check_4B436630Y_live_real_micro_canary_reconciliation.py"), "--once-json"], cwd=root, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env, check=False, timeout=300)
    try:
...
def main() -> int:
    ...
    print("4B.4.3.6.6.30Y live-real micro canary reconciliation applied")
    checks = checker_report.get("checks", {}) if isinstance(checker_report.get("checks"), dict) else {}
    for key in ("base_30x_checker_ok", "module_probe_ok", "module_probe_mismatch_count_zero", "module_probe_patch_network_submit_attempted", "module_probe_no_additional_exchange_submit"):
