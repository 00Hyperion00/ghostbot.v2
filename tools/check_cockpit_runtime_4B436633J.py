...
def _request_json(url: str, token: str, operator: str) -> dict[str, Any]:
    request = urllib.request.Request(
        url,
        headers={"X-TradeBot-Auth": token, "X-TradeBot-Operator": operator},
        method="GET",
    )
    with urllib.request.urlopen(request, timeout=5) as response:  # noqa: S310 - local operator helper
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check 33J cockpit recovery plan apply verification gate.")
    ...
    apply_gate = snapshot.get("recovery_plan_apply_verification_gate") or runtime_awareness.get("recovery_plan_apply_verification_gate") or recovery_gate.get("recovery_plan_apply_verification_gate") or {}
