...
from tradebot.research_hyp004_cross_symbol_relative_strength_exploration import (  # noqa: E402
...
def http_get_json(base_url: str, path: str, params: dict[str, Any], timeout_sec: float) -> Any:
    ...
    request = Request(url, headers={"User-Agent": "tradebot-25O-hyp004-research/1.0"}, method="GET")
...
def latest_25n_report(reports_dir: Path) -> Path | None:
    matches = sorted(reports_dir.glob("4B436625N_research_backlog_after_hyp003_closure_*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    return matches[0] if matches else None


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="4B.4.3.6.6.25O HYP-004 cross-symbol relative strength exploration gate")
    parser.add_argument("--input-json", default=None, help="25N backlog advancement report selecting HYP-004.")
    parser.add_argument("--input-csv", default=None, help="Offline CSV with symbol/open_time/close columns.")
    parser.add_argument("--reports-dir", default="reports", help="Directory used to discover latest 25N report if --input-json is omitted.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.review_ok:
        print("ERROR: --review-ok is required. HYP-004 exploration is research-only and cannot approve paper/live trading.", file=sys.stderr)
        return 2
    input_json = Path(args.input_json) if args.input_json else latest_25n_report(Path(args.reports_dir))
    selection_report = load_json(input_json) if input_json and input_json.exists() else None
    symbols = [s.strip().upper() for s in args.symbols.split(",") if s.strip()]
    if len(symbols) < 2:
        print("ERROR: at least two symbols are required for HYP-004 cross-symbol exploration", file=sys.stderr)
        return 2
    if args.input_csv:
    ...
    print(f"{HYP004_EXPLORATION_CONTRACT_VERSION} HYP-004 relative strength exploration {report['decision']}")
