...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="TradeBot V2 read-only operator cockpit shadow audit visualization pack")
    parser.add_argument("--project-root", type=Path, default=PROJECT_ROOT)
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8090)
    parser.add_argument("--open-browser", action="store_true")
    parser.add_argument("--allow-non-loopback", action="store_true")
    parser.add_argument("--once-json", action="store_true", help="Print one read-only visualization snapshot and exit")
...
def main() -> int:
    ...
    print(f"{OPERATOR_COCKPIT_V2_VISUALIZATION_PACK_VERSION} Operator Cockpit V2 shadow audit visualization pack")
    print(f" - url: {url}")
