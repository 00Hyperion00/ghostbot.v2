...
def _run_checker(root: Path) -> dict[str, Any]:
    ...
    proc = subprocess.run([sys.executable, str(root / "tools/check_4B436631A_live_micro_canary_freeze_audit_closure.py"), "--once-json"], cwd=root, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env, check=False, timeout=300)
    try:
...
def main() -> int:
    ...
    print("4B.4.3.6.6.31A live micro-canary freeze and audit closure applied")
