...
def main() -> int:
    ...
    results["read_only_contract_present"] = "read_only" in text and "trading_action_performed" in text
    results["stagnation_detector_present"] = "UNCHANGED_LEDGER_PAYLOAD_ACROSS_RECENT_RUNS" in text
    ok = ok and bool(results["contract_version_present"]) and bool(results["read_only_contract_present"]) and bool(results["stagnation_detector_present"])

    print(f"{CONTRACT_VERSION} HYP-006 signal frequency stagnation diagnostics patch applied")
