...
# CLI_MARKERS_25T: __input_json __input_csv __symbols __interval __review_ok public_market_data_GET_only
import argparse
import json
import sys
import time
...
from tradebot.research_hyp005_robustness_walkforward import (  # noqa: E402
...
def http_get_json(base_url: str, path: str, params: dict[str, Any], timeout_sec: float) -> Any:
    ...
    request = Request(url, headers={"User-Agent": "tradebot-25T-hyp005-robustness/1.0"}, method="GET")
...
def latest_25s_report(reports_dir: Path) -> Path | None:
    matches = sorted(reports_dir.glob("4B436625S_hyp005_liquidity_sweep_reversal_exploration_*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
    return matches[0] if matches else None


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="4B.4.3.6.6.25T HYP-005 robustness / walk-forward confirmation gate")
    parser.add_argument("--input-json", default=None, help="25S exploration PASS report selecting HYP-005 candidate.")
    parser.add_argument("--input-csv", default=None, help="Offline CSV with symbol/open_time/open/high/low/close columns.")
    parser.add_argument("--reports-dir", default="reports", help="Directory used to discover latest 25S report if --input-json is omitted.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.review_ok:
        print("ERROR: --review-ok is required. HYP-005 robustness is research-only and cannot approve paper/live trading.", file=sys.stderr)
        return 2
    input_json = Path(args.input_json) if args.input_json else latest_25s_report(Path(args.reports_dir))
    exploration_report = load_json(input_json) if input_json and input_json.exists() else None
    symbols = [s.strip().upper() for s in args.symbols.split(",") if s.strip()]
    if not symbols:
        print("ERROR: at least one symbol is required for HYP-005 robustness", file=sys.stderr)
        return 2
    if args.input_csv:
    ...
    print(f"{HYP005_ROBUSTNESS_CONTRACT_VERSION} HYP-005 robustness/walk-forward {report['decision']}")
    ...
    print(f" - penalized_mean_net_edge_bps: {metrics.get('penalized_mean_net_edge_bps')}")
    print(f" - median_net_edge_bps: {metrics.get('median_net_edge_bps')}")
    print(f" - profit_factor: {metrics.get('profit_factor')}")
    print(f" - walk_forward_positive_rate_pct: {metrics.get('walk_forward_positive_rate_pct')}")
    print(f" - oos_mean_net_edge_bps: {metrics.get('oos_mean_net_edge_bps')}")
    print(f" - top_win_dependency_pct: {metrics.get('top_win_dependency_pct')}")
    print(f" - dominant_symbol_pct: {metrics.get('dominant_symbol_pct')}")
    print(f" - wick_dependency_pct: {metrics.get('wick_dependency_pct')}")
