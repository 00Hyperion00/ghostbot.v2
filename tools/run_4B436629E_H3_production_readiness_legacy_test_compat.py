...
from check_4B436629E_H3_production_readiness_legacy_test_compat import CONTRACT_VERSION, build_report


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate 4B.4.3.6.6.29E-H3 production readiness legacy test compatibility decision report")
    parser.add_argument("--reports-dir", default="reports/production_hardening")
    args = parser.parse_args()
    root = Path.cwd()
    report = build_report(root)
    now = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    decision = "PRODUCTION_READINESS_LEGACY_TEST_COMPAT_READY_LIVE_REAL_STILL_BLOCKED" if report.get("ok") else "PRODUCTION_READINESS_LEGACY_TEST_COMPAT_NOT_READY"
    payload = {
        ...
        "legacy_h1_test_compat_restored": bool(report.get("checks", {}).get("legacy_h1_test_actual_decision_present")),
        "approved_for_evidence_merge_baseline": bool(report.get("sample_probe", {}).get("evidence_complete")),
        "approved_for_paper_candidate_preflight": bool(report.get("sample_probe", {}).get("paper_candidate_preflight_ready")),
    ...
    json_path = reports_dir / f"4B436629E_H3_production_readiness_legacy_test_compat_decision_{now}.json"
    md_path = json_path.with_suffix(".md")
    json_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8", newline="\n")
    md_path.write_text(
        "# 4B.4.3.6.6.29E-H3 Production Readiness Legacy Test Compatibility\n\n"
    ...
    print(f"{CONTRACT_VERSION} Production Readiness Legacy Test Compatibility {decision}")
    for key in (
        "read_only",
