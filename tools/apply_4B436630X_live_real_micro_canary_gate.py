...
CONFIG_FIELDS = [
    ...
    "live_real_micro_canary_reduce_only",
    "live_real_micro_canary_post_only",
    "live_real_micro_canary_time_in_force",
...
def _run_checker(root: Path) -> dict[str, Any]:
    ...
    proc = subprocess.run(
        [sys.executable, str(root / "tools/check_4B436630X_live_real_micro_canary_gate.py"), "--once-json"],
        cwd=root,
        text=True,
...
def main() -> int:
    ...
    payload = {
        ...
        "approved_for_exchange_submit": bool(checker_report.get("approved_for_exchange_submit")),
        "approved_for_live_real": bool(checker_report.get("approved_for_live_real")),
    ...
    print("4B.4.3.6.6.30X first live-real micro canary gate applied")
    checks = checker_report.get("checks", {}) if isinstance(checker_report.get("checks"), dict) else {}
    for key in (
        "base_30w_checker_ok",
        "module_probe_ok",
        "module_probe_approved_for_exchange_submit",
        "module_probe_network_submit_attempted",
        "module_probe_live_real_order_performed",
    ):
