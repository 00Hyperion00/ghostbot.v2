...
def _run_checker(root: Path) -> dict[str, Any]:
    ...
    proc = subprocess.run([sys.executable, str(root / "tools/check_4B436630O_H2_reconciliation_checker_probe_signature_hotfix.py"), "--once-json"], cwd=root, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env, check=False, timeout=300)
    try:
...
def main() -> int:
    ...
    print("4B.4.3.6.6.30O-H2 reconciliation checker probe signature hotfix applied")
    for key in ("target_30o_checker_ok", "h1_checker_ok", "target_mismatch_zero", "target_sqlite_mirror_ok", "target_exchange_submit_blocked", "target_live_real_blocked"):
