...
CONFIG_FIELDS = [
    "release_hygiene_bad_evidence_cleanup_enabled",
    "release_hygiene_bad_evidence_quarantine_required",
    "release_hygiene_final_audit_snapshot_required",
    "release_hygiene_no_further_live_orders_required",
    "release_hygiene_finalization_token",
]
CONFIG_BLOCK = """
...
def _run_checker(root: Path) -> dict[str, Any]:
    ...
    proc = subprocess.run(
        [sys.executable, str(root / "tools/check_4B436631B_release_hygiene_bad_evidence_ledger_cleanup.py"), "--once-json"],
        cwd=root,
        text=True,
...
def main() -> int:
    ...
    print("4B.4.3.6.6.31B release hygiene / bad evidence ledger cleanup tooling applied")
    checks = checker_report.get("checks", {}) if isinstance(checker_report.get("checks"), dict) else {}
    for key in ("ready_decision_ok", "source_31a_h3_verified", "bad_evidence_quarantined", "moved_file_count_ok", "patch_network_submit_false"):
