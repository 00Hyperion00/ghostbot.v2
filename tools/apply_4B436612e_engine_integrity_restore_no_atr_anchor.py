...
def safe_latest_mark_price(text: str) -> str:
    # 12e: restored engine candidates may not have _latest_atr.
    # Replace _latest_mark_price if it exists; otherwise insert before the first stable
    # TradeBotEngine method that exists. Never use a greedy regex across class methods.
    span = find_class_method_span(text, '_latest_mark_price')
    if span:
        start, end = span
        return text[:start] + SAFE_LATEST_MARK_PRICE.rstrip() + '\n\n' + text[end:].lstrip('\n')
    for anchor in (
        '_latest_atr',
        '_risk_plan_execution_snapshot',
        '_protective_exit_snapshot',
        '_position_snapshot',
        '_pending_snapshot',
        'get_status',
        '_health_snapshot',
    ):
        before_span = find_class_method_span(text, anchor)
        if before_span:
            start, _ = before_span
            return text[:start] + SAFE_LATEST_MARK_PRICE.rstrip() + '\n\n' + text[start:]
    raise RuntimeError('Cannot find any safe insertion point for _latest_mark_price')
