...
EXPECTED_FILES = [
    "README_APPLY_4B436630Y_H1.txt",
    "docs/LIVE_REAL_MICRO_CANARY_RECONCILIATION_4B436630Y_H1.md",
...
import json
print(json.dumps({
    "ok": payload["decision"] == READY_DECISION,
    "decision": payload["decision"],
    "mismatch_count": payload["mismatch_count"],
    "fill_reconciliation_verified": payload["fill_reconciliation_verified"],
    "manual_min_notional_quantity_adjustment_accepted": payload["reconciliation"].get("manual_min_notional_quantity_adjustment_accepted"),
    "patch_network_submit_attempted": payload["patch_network_submit_attempted"],
    "approved_for_additional_exchange_submit": payload["approved_for_additional_exchange_submit"],
}))
'''
...
def build_report(root: Path) -> dict[str, Any]:
    ...
    base_30x = _run_json_tool(root, "tools/check_4B436630X_live_real_micro_canary_gate.py")
    base_30y = _run_json_tool(root, "tools/check_4B436630Y_live_real_micro_canary_reconciliation.py")
    ...
    checks = {
        ...
        "base_30y_checker_informational": bool(base_30y.get("ok")) or bool(base_30y.get("skipped")),
        "module_probe_ok": bool(probe.get("ok")),
        "module_probe_mismatch_count_zero": probe.get("mismatch_count") == 0,
        "module_probe_adjustment_accepted": probe.get("manual_min_notional_quantity_adjustment_accepted") is True,
    ...
    return {
        "ok": all(value for key, value in checks.items() if key != "base_30y_checker_informational"),
