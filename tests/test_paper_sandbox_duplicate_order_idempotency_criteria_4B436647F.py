...
from tradebot.paper_sandbox_duplicate_order_idempotency_criteria import evaluate
...
def test_paper_sandbox_duplicate_order_idempotency_criteria_ready_and_locked(tmp_path: Path) -> None:
    ...
    assert payload["patch_id"] == "4B436647F"
    assert payload["patch_version"] == "4B.4.3.6.6.47F"
