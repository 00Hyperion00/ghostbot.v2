...
from tradebot.paper_sandbox_observation_runtime_metrics import (
...
def write_source_report(reports_dir: Path, *, overrides: dict[str, object] | None = None, name_suffix: str = "ready") -> Path:
    ...
    path = reports_dir / f"4B436639E_paper_sandbox_local_runtime_health_probe_evidence_20260706T115345Z_{name_suffix}.json"
    path.write_text(json.dumps(payload, indent=2, sort_keys=True), encoding="utf-8")
    return path


def test_ready_report_from_valid_39e_source(tmp_path: Path) -> None:
    reports = tmp_path / "reports" / "recovery"
    source = write_source_report(reports)
    report = build_report(reports)
    assert report["status"] == "READY"
    assert report["decision"] == READY_DECISION
    assert report["source_39e_status"] == "SOURCE_39E_READY"
    assert report["source_39e_report"] == str(source)
...
def test_missing_source_fails_closed(tmp_path: Path) -> None:
    ...
    assert report["source_39e_status"] == "SOURCE_39E_MISSING"
...
def test_not_ready_source_fails_closed(tmp_path: Path) -> None:
    ...
    report = build_report(reports)
    assert report["status"] == "NOT_READY"
    assert report["source_39e_status"] == "SOURCE_39E_NOT_READY"
    assert report["approved_for_paper_sandbox_observation_runtime_metrics"] is False


def test_latest_main_ready_report_selection_ignores_artifacts(tmp_path: Path) -> None:
    reports = tmp_path / "reports" / "recovery"
    main = write_source_report(reports)
    artifact = reports / "4B436639E_paper_sandbox_local_runtime_health_probe_evidence_probe_20260706T115345Z_ready.json"
    artifact.write_text(json.dumps({"status": "READY"}), encoding="utf-8")
    selected = find_latest_source_report(reports)
    assert selected == main


def test_observation_runtime_metrics_contract_declared_only(tmp_path: Path) -> None:
    reports = tmp_path / "reports" / "recovery"
    write_source_report(reports)
    report = build_report(reports)
    assert report["observation_runtime_metrics_contract_complete"] is True
    assert report["observation_runtime_metrics_contract_locked"] is True
    assert report["observation_runtime_metrics_contract_ready"] is True
    assert report["observation_runtime_metrics_contract_only"] is True
    assert report["observation_runtime_metrics_schema_declared"] is True
    assert report["future_observation_runtime_metrics_sample_declared"] is True
    assert tuple(report["observation_runtime_metrics_required_fields"]) == METRICS_REQUIRED_FIELDS


def test_metrics_collection_not_performed_and_runtime_not_started(tmp_path: Path) -> None:
    reports = tmp_path / "reports" / "recovery"
    write_source_report(reports)
    report = build_report(reports)
    assert report["runtime_process_status"] == "NOT_STARTED_BY_39F"
    assert report["runtime_process_started"] is False
...
def test_probe_counts_are_locked(tmp_path: Path) -> None:
    ...
    write_source_report(reports)
    report = build_report(reports)
    assert report["paper_sandbox_observation_runtime_metrics_probe_count"] == 21
    assert report["paper_sandbox_observation_runtime_metrics_probe_passed_count"] == 21
    assert report["paper_sandbox_observation_runtime_metrics_check_count"] == 28
    assert report["paper_sandbox_observation_runtime_metrics_ready_count"] == 28
...
def test_write_artifacts_creates_ready_report(tmp_path: Path) -> None:
    ...
    assert Path(report["observation_runtime_metrics_contract_path"]).exists()
    assert Path(report["observation_runtime_metrics_schema_path"]).exists()
    assert Path(report["future_observation_runtime_metrics_sample_path"]).exists()
...
def test_next_phase_not_auto_unlocked(tmp_path: Path) -> None:
    ...
    write_source_report(reports)
    report = build_report(reports)
    assert report["next_phase"] == "4B.4.3.6.6.39G"
