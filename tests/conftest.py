    ...
    try:
        ...
        if module is None:
            try:
                ...
                module.__file__ = getattr(compat, "__file__", None)
                sys.modules["tradebot.production_hardening"] = module
        for name in getattr(compat, "__all__", ()):  # includes RuntimeLockHandle
            setattr(module, name, getattr(compat, name))
    except Exception:
        pass
# --- end 4B436661_H7 pytest runtime lock handle import finalization ---
