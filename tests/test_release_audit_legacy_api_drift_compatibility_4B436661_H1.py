...
from tradebot.release_audit_legacy_api_drift_compatibility import (  # noqa: E402
    DECISION,
    build_legacy_api_drift_compatibility_snapshot,
    verify_legacy_api_contracts,
)

def test_phase61_h1_snapshot_ready() -> None:
    snapshot = build_legacy_api_drift_compatibility_snapshot(ROOT)
    assert snapshot["ok"] is True
    assert snapshot["status"] == "READY"
    assert snapshot["phase_61_h1_closed"] is True
    assert snapshot["decision"] == DECISION
    assert snapshot["legacy_api_contract_ready_count"] == 3
    assert snapshot["missing_legacy_api_contracts"] == []

def test_phase61_h1_restores_required_imports() -> None:
    from tradebot.paper_sandbox_execution_reconciliation_gate import SQLITE_MIRROR_REQUIRED_DECISION
    from tradebot.production_hardening import build_production_hardening_snapshot
    from tradebot.operator_cockpit_v2_read_only import OPERATOR_COCKPIT_V2_RISK_SIZING_AUDIT_PARITY

    assert isinstance(SQLITE_MIRROR_REQUIRED_DECISION, str)
    assert "SQLITE_MIRROR" in SQLITE_MIRROR_REQUIRED_DECISION
...
def test_phase61_h1_contract_verifier() -> None:
...
def test_phase61_h1_safety_locks() -> None:
    snapshot = build_legacy_api_drift_compatibility_snapshot(ROOT)
    locked = [
